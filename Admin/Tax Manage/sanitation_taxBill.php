...
if(isset($_GET['id'])) {
    try {
        ...
        error_log("Database Error in sanitation_taxBill.php: " . $e->getMessage());
...
$previousTotal = isset($propertyData['previous_sanitation_tax']) ? floatval($propertyData['previous_sanitation_tax']) : 0;
$currentTotal = isset($propertyData['sanitation_tax']) ? floatval($propertyData['sanitation_tax']) : 0;
...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO sanitation_tax_bills (property_id, previous_total, current_total, discount, penalty, due_amount, total_amount, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
    if (!$stmt) {
        throw new Exception('Prepare Failed: ' . $conn->error);
    }
    ...
    error_log("Database Insert Error in sanitation_taxBill.php: " . $e->getMessage());
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Sanitation Tax Receipt</title>
    <style>
        body {
    ...
    function downloadAsPDF() {
        ...
        const opt = {
            margin: 1,
            filename: 'sanitation_tax_bill_<?php echo isset($propertyData["id"]) ? $propertyData["id"] : ""; ?>.pdf',
...
<body>
    <div class="receipt">
        ...
        <div class="title">Sanitation Tax Receipt</div>
